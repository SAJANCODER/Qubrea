...
# --- AI & TELEGRAM ---
def generate_ai_analysis(commit_data, files_changed, diff_content=None):
    """
    Enhanced to analyze actual code changes instead of just commit messages
    """
    commit_msg = commit_data.get('message', 'No message.')
    
    # Build input based on whether we have actual code diff
    if diff_content:
        # We have actual code changes - AI will analyze the code itself
        input_text = f"""
ACTUAL CODE CHANGES (DIFF):
{diff_content}

FILES MODIFIED: {', '.join(files_changed)}
Developer's commit message (for reference only): {commit_msg}
"""
        
        prompt = f"""
You are an expert code reviewer with deep technical knowledge. Analyze the ACTUAL CODE CHANGES shown in the diff below.

{input_text}

CRITICAL INSTRUCTIONS:
1. **ANALYZE THE CODE CHANGES DIRECTLY** - Look at what was added (+) and removed (-) in the code
2. **IGNORE the developer's commit message** - Form your own understanding from the code
3. Identify the core technical concepts being changed:
   - What algorithms, data structures, or patterns are being modified?
   - What business logic or functionality is being added/removed?
   - Are there API changes, refactorings, bug fixes, or new features?
4. Think like a senior developer reviewing the actual code implementation
5. Return valid HTML ONLY (Telegram-compatible)
6. DO NOT USE <ul>, <ol>, or <li> tags - use "•" character for bullets
7. Use <b> for bold, <i> for italic, <code> for inline code
8. Use <br> or newlines for line breaks

OUTPUT FORMAT:
<b>Code Analysis:</b>
[2-3 sentence summary of what was ACTUALLY changed in the code - be specific about technical concepts]

<b>Technical Changes:</b>
• [Specific change 1 - describe the actual logic/functionality modified]
• [Specific change 2 - focus on what the code does, not just file names]
• [Specific change 3 if applicable]

<b>Impact:</b>
[What this code change enables, fixes, or improves]
"""
    else:
        # Fallback: no diff available, analyze based on files and commit message
        input_text = f"FILES CHANGED: {', '.join(files_changed)}\nCOMMIT MESSAGE: {commit_msg}"
        
        prompt = f"""
You are an AI Code Reviewer. Analyze this commit based on available information.
...
OUTPUT FORMAT:
<b>Review Status:</b> [Status based on file patterns and commit message]
<b>Summary:</b> [Inferred changes from filenames and message]
<b>Files Modified:</b>
• [List files with inferred purpose]
<i>Note: Analysis based on filenames only - actual code diff not available</i>
"""
...
# Add this helper function to extract diff content from GitHub API response
def extract_diff_from_files(files_info, max_lines=100):
    """
    Extract meaningful code changes from GitHub API file objects
    Limits to max_lines to avoid token limits
    """
    diff_parts = []
    total_lines = 0
    
    for file_info in files_info[:10]:  # Limit to first 10 files
        ...
        if patch:
            ...
            for line in patch_lines:
                if total_lines >= max_lines:
                    break
                # Include context and actual changes
...
def process_standup_task(target_bot_token, target_chat_id, author_name, data):
    try:
        ...
        if compare_data:
            ...
            # NEW: Extract actual code diff from files
            diff_content = extract_diff_from_files(files_info, max_lines=80)
            
            head_commit = data.get('head_commit') or (commits[0] if commits else {})
            # Pass diff_content to AI for code-based analysis
            ai_response = generate_ai_analysis(head_commit or {}, files_list, diff_content=diff_content)
            ...
            for commit in commits:
                ...
                # Try to fetch diff for individual commit if token available
                diff_content = None
                commit_sha = commit.get('id')
                token = get_decrypted_token_for_chat(target_chat_id)
                if owner_login and repo_name and commit_sha and token:
                    # Fetch commit details with patch
                    try:
                        url = f"https://api.github.com/repos/{owner_login}/{repo_name}/commits/{commit_sha}"
                        headers = {"Authorization": f"Bearer {token}", "Accept": "application/vnd.github.v3+json"}
                        r = requests.get(url, headers=headers, timeout=12)
                        if r.status_code == 200:
                            commit_detail = r.json()
                            if 'files' in commit_detail:
                                diff_content = extract_diff_from_files(commit_detail['files'], max_lines=80)
                    except Exception as e:
                        print(f"Failed to fetch commit diff: {e}")
                
                # Generate AI analysis with diff if available
                ai_response = generate_ai_analysis(commit, files_list, diff_content=diff_content)
                summary = ai_response.strip()
                commit_id = commit.get('id', 'unknown')[:7]
                confidence = "code-analyzed" if diff_content else confidence_tag
...
def dashboard():
    ...
    try:
        ...
        # NEW: Get active branches
        c.execute("""
            SELECT DISTINCT branch_name, COUNT(*) as commit_count, 
        ...
        for row in c.fetchall():
            branch_name, commit_count, last_activity = row
            last_active = last_activity.astimezone(IST) if last_activity else now_ist
            minutes_ago = int((now_ist - last_active).total_seconds() / 60)
            active_branches.append({
                'name': branch_name,
                'commits': commit_count,
                'last_active': f"{minutes_ago}m ago" if minutes_ago < 60 else f"{minutes_ago//60}h ago",
                'is_active': minutes_ago < 30  # Active if < 30 minutes
            })

        # Get currently working developers (active in last 30 minutes)
        ...
        c.execute("""
            SELECT DISTINCT author, 
                   ...
                   COUNT(*) as recent_commits
            FROM project_updates 
            WHERE chat_id = %s AND timestamp >= %s
            GROUP BY author
            ORDER BY last_commit DESC
        """, (str(target_chat_id), recent_threshold))
        
        online_developers = []
        for row in c.fetchall():
            author, last_commit, recent_commits = row
            last_commit_time = last_commit.astimezone(IST) if last_commit else now_ist
            minutes_ago = int((now_ist - last_commit_time).total_seconds() / 60)
            online_developers.append({
                'name': author,
                'last_active': f"{minutes_ago}m ago",
                'recent_commits': recent_commits,
                'status': 'online' if minutes_ago < 15 else 'away' if minutes_ago < 60 else 'offline'
            })
        ...
        online_count = len([d for d in online_developers if d['status'] in ['online', 'away']])
